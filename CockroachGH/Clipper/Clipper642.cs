  ...
  //assumes that e1 will be to the Right of e2 ABOVE the intersection
    ...
    private void JoinCommonEdges()
      ...
      internal static Paths Minkowski(Path pattern, Path path, bool IsSum, bool IsClosed)
      {
        ...
        private static Path TranslatePathToZero(Path path, IntPoint delta) {
            Path outPath = new Path(path.Count);
            for (int i = 0; i < path.Count; i++)
                outPath.Add(new IntPoint(path[i].X - delta.X, path[i].Y - delta.Y));
            return outPath;
        ...
        public static Paths MinkowskiDiffInner(Path pattern, Paths paths, bool pathIsClosed) {
            Paths solution = new Paths();
            Clipper c = new Clipper();
            Paths input = new Paths();
            Path patternTranslatedTo0 = TranslatePathToZero(pattern, pattern[0]);


            for (int i = 0; i < paths.Count; ++i) {
                Paths tmp = Minkowski(pattern, paths[i], false, pathIsClosed);
                c.AddPaths(tmp, PolyType.ptSubject, true);
                if (pathIsClosed) {
                    Path path = TranslatePath(paths[i], patternTranslatedTo0[0]);
            ...
            Path path0 = TranslatePath(paths[0], patternTranslatedTo0[0]);
            solution.RemoveAt(0);
        ...
        //Multiple Minkowski Diff for nested objects
        public static Paths MinkowskiDiffInner(Path pattern, List<Paths> paths, bool pathIsClosed = true) {

            Paths solution = new Paths();
            Clipper c = new Clipper();
            for (int i = 0; i < paths.Count; ++i) {
                Paths tmp = MinkowskiDiff(pattern, paths[i], pathIsClosed);
                c.AddPaths(tmp, PolyType.ptSubject, true);
            }
        ...
        public static Paths MinkowskiDiff(Path pattern, Paths paths, bool pathIsClosed) {
            Paths solution = new Paths();
            Clipper c = new Clipper();
            Path patternTranslatedTo0 = TranslatePathToZero(pattern, pattern[0]);
                ...
                for (int i = 0; i < paths.Count; ++i) {
                    ...
                    Paths tmp = Minkowski(patternTranslatedTo0, paths[i], false, pathIsClosed);
                    c.AddPaths(tmp, PolyType.ptSubject, true);
                    if (pathIsClosed) {
                        Path path = TranslatePath(paths[i], patternTranslatedTo0[0]);
                        c.AddPath(path, PolyType.ptClip, true);
        ...
        //Multiple Minkowski Diff for nested objects
        public static Paths MinkowskiDiff(Path pattern, List<Paths> paths, bool pathIsClosed = true) {
        ...
        public static Paths MinkowskiDiffParallel(Path pattern, List<Paths> paths, bool pathIsClosed = true) {

           
            int n = paths.Count;
            var pathsArray = paths.ToArray();
            var results = new Paths[n];

           System.Threading.Tasks.Parallel.For(0, n, i => {
               results[i] = MinkowskiDiff(pattern, pathsArray[i], pathIsClosed);
            });
